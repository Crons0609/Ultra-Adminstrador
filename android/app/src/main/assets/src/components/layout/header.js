...
export class Header extends Component {
    ...
    return `
      <header class="header" id="main-header">
        ...
        <div class="header-right">
          ...
          <button class="header-action" id="header-notif-btn" title="Notificaciones" aria-label="Notificaciones" style="position:relative;">
  ...
  afterMount() {
    ...
    const notifBadge = this.$('#notif-badge-count');
    const currentUser = GlobalStore.getState().currentUser;

    if (notifBtn) {
      notifBtn.addEventListener('click', () => NotificationCenter.toggle());
    }

    if (currentUser?.uid && notifBadge) {
      this.unsubscribeNotifCount = PushNotificationsCenterService.subscribeUnreadCount(
        currentUser.uid,
        (count) => {
          if (count > 0) {
