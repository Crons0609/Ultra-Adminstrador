...
export class PublicCatalogView extends Component {
  ...
  mount() {
    ...
    const langUnsub = I18nService.subscribe(() => {
      this.state.activeCategory = I18nService.t('pub_cat_all');
      this.renderCatalog(root);
    });
    this.listeners.push(langUnsub);
  ...
  loadCatalogData(root) {
    if (!this.companyId) {
      root.innerHTML = `<div class="pub-container text-center py-10"><p>${I18nService.t('pub_not_specified')}</p></div>`;
      return;
    }
    ...
    const productsListener = FirestoreService.listenToPath(`${this.companyId}/productos`, (products) => {
      ...
      const uniqueCats = [I18nService.t('pub_cat_all'), ...new Set(this.state.products.map(p => p.category).filter(Boolean))];
  ...
  renderCatalog(root) {
    ...
    const socialLinks = [
      socials.whatsapp ? `<a href="https://wa.me/${socials.whatsapp}" target="_blank" rel="noopener" class="pub-social-btn btn-whatsapp-click" title="${I18nService.t('pub_whatsapp_title')}"><svg viewBox="0 0 24 24" fill="currentColor" width="16" height="16"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg></a>` : '',
      ...
      socials.website ? `<a href="${socials.website}" target="_blank" rel="noopener" class="pub-social-btn btn-social-click" title="${I18nService.t('sh_format_custom')}"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="16" height="16"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg></a>` : '',
    ].filter(Boolean).join('');
    ...
    root.innerHTML = `
      ...
      <main class="pub-container flex-grow" style="padding-top:var(--space-5);">
        ...
        <div class="pub-toolbar">
          <div class="pub-search-wrap">
            ...
            <input type="text" id="pub-search" class="pub-search-input" placeholder="${this.state.businessCategory === 'SUPERMERCADO_TIENDA' ? I18nService.t('pub_search_placeholder_store') : I18nService.t('pub_search_placeholder_generic')}" value="${this.state.searchQuery}" autocomplete="off" />
          </div>
          <select id="pub-sort" class="pub-sort-select">
            <option value="recent" ${this.state.sortBy === 'recent' ? 'selected' : ''}>${I18nService.t('pub_sort_recent')}</option>
            <option value="price-asc" ${this.state.sortBy === 'price-asc' ? 'selected' : ''}>${I18nService.t('pub_sort_price_asc')}</option>
            <option value="price-desc" ${this.state.sortBy === 'price-desc' ? 'selected' : ''}>${I18nService.t('pub_sort_price_desc')}</option>
            <option value="promo" ${this.state.sortBy === 'promo' ? 'selected' : ''}>${I18nService.t('pub_sort_promo')}</option>
          </select>
          ${this.state.businessCategory === 'SUPERMERCADO_TIENDA' ? `
            <button id="pub-toggle-stock" class="pub-cat-pill" style="white-space:nowrap; flex-shrink:0; font-size:0.75rem;">
              ${this.state.showOutOfStock ? I18nService.t('pub_hide_out_of_stock') : I18nService.t('pub_show_out_of_stock')}
            </button>
          ` : ''}
        </div>
      ...
      <footer class="pub-footer">
        <p>${I18nService.t('pub_footer_rights', { year: new Date().getFullYear(), company: companyName })}</p>
      </footer>

      <!-- Floating WhatsApp -->
      ${socials.whatsapp ? `
        <a href="https://wa.me/${socials.whatsapp}" target="_blank" rel="noopener"
          class="pub-float-wa btn-whatsapp-click" title="${I18nService.t('pub_whatsapp_title')}" aria-label="WhatsApp">
  ...
  renderServiceRequestSection(root) {
    ...
    const extraFields = isCarpinteria ? `
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:var(--space-3);">
        <div><label style="font-size:0.8rem;font-weight:600;display:block;margin-bottom:4px;color:var(--pub-text);">${I18nService.t('pub_sr_measures')}</label>
          <input type="text" id="req-medidas" class="pub-form-input" placeholder="${I18nService.t('pub_sr_measures_placeholder')}" /></div>
        <div><label style="font-size:0.8rem;font-weight:600;display:block;margin-bottom:4px;color:var(--pub-text);">${I18nService.t('pub_sr_material')}</label>
          <input type="text" id="req-material" class="pub-form-input" placeholder="${I18nService.t('pub_sr_material_placeholder')}" /></div>
        <div><label style="font-size:0.8rem;font-weight:600;display:block;margin-bottom:4px;color:var(--pub-text);">${I18nService.t('pub_sr_finish')}</label>
          <input type="text" id="req-color" class="pub-form-input" placeholder="${I18nService.t('pub_sr_finish_placeholder')}" /></div>
      </div>
    ` : isCamaras ? `
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:var(--space-3);">
        <div><label style="font-size:0.8rem;font-weight:600;display:block;margin-bottom:4px;color:var(--pub-text);">${I18nService.t('pub_sr_phone')}</label>
          <input type="tel" id="req-telefono" class="pub-form-input" placeholder="${I18nService.t('pub_sr_phone_placeholder')}" /></div>
        <div><label style="font-size:0.8rem;font-weight:600;display:block;margin-bottom:4px;color:var(--pub-text);">${I18nService.t('pub_sr_location')}</label>
          <input type="text" id="req-ubicacion" class="pub-form-input" placeholder="${I18nService.t('pub_sr_location_placeholder')}" /></div>
        <div><label style="font-size:0.8rem;font-weight:600;display:block;margin-bottom:4px;color:var(--pub-text);">${I18nService.t('pub_sr_camera_count')}</label>
          <input type="number" id="req-num-camaras" class="pub-form-input" placeholder="${I18nService.t('pub_sr_camera_count_placeholder')}" min="1" /></div>
      </div>
    ` : '';
    ...
    section.innerHTML = `
      <div class="pub-service-card">
        <h2 class="pub-section-title">
          ${isCarpinteria ? I18nService.t('pub_sr_title_carpentry') : isCamaras ? I18nService.t('pub_sr_title_cameras') : I18nService.t('pub_sr_title_quote')}
        </h2>
        <p style="font-size:0.85rem;color:var(--pub-text-sec);margin-bottom:var(--space-4);">
          ${I18nService.t('pub_sr_subtitle')}
        </p>
        <form id="pub-service-request-form" style="display:flex;flex-direction:column;gap:var(--space-3);">
          <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:var(--space-3);">
            <div><label style="font-size:0.8rem;font-weight:600;display:block;margin-bottom:4px;color:var(--pub-text);">${I18nService.t('pub_sr_client_name')}</label>
              <input type="text" id="req-client-name" class="pub-form-input" placeholder="${I18nService.t('pub_sr_client_name_placeholder')}" required /></div>
            <div><label style="font-size:0.8rem;font-weight:600;display:block;margin-bottom:4px;color:var(--pub-text);">${I18nService.t('pub_sr_client_email')}</label>
              <input type="email" id="req-client-email" class="pub-form-input" placeholder="tu@correo.com" /></div>
          </div>
          ${extraFields}
          <div><label style="font-size:0.8rem;font-weight:600;display:block;margin-bottom:4px;color:var(--pub-text);">${I18nService.t('pub_sr_description')}</label>
            <textarea id="req-description" class="pub-form-input" style="min-height:90px;resize:vertical;" placeholder="${I18nService.t('pub_sr_description_placeholder')}" required></textarea>
          </div>
          ...
          <button type="submit" id="pub-req-submit-btn" class="pub-submit-btn">${I18nService.t('pub_sr_submit')}</button>
        </form>
      </div>
    `;
    ...
    if (form) {
      form.addEventListener('submit', async (e) => {
        ...
        if (submitBtn) { submitBtn.disabled = true; submitBtn.textContent = I18nService.t('pub_sr_submitting'); }
        ...
        try {
          ...
          if (feedback) {
            ...
            feedback.textContent = I18nService.t('pub_sr_success');
          }
          form.reset();
        } catch (err) {
          if (feedback) {
            ...
            feedback.textContent = I18nService.t('pub_sr_error');
          }
        } finally {
          if (submitBtn) { submitBtn.disabled = false; submitBtn.textContent = I18nService.t('pub_sr_submit'); }
        }
      });
    }
  }
  ...
  bindEvents(root) {
    ...
    if (toggleStockBtn) {
      toggleStockBtn.addEventListener('click', () => {
        ...
        toggleStockBtn.textContent = this.state.showOutOfStock ? I18nService.t('pub_hide_out_of_stock') : I18nService.t('pub_show_out_of_stock');
        this.renderProducts(root);
      });
    }
  ...
  renderProducts(root) {
    ...
    let filtered = products.filter(p => {
      const matchesCategory = activeCategory === I18nService.t('pub_cat_all') || p.category === activeCategory;
    ...
    if (filtered.length === 0) {
      grid.innerHTML = `
        <div class="pub-empty-state" style="grid-column:1/-1;">
          ...
          <p class="pub-empty-title">${I18nService.t('pub_empty_results_title')}</p>
          <p class="pub-empty-desc">${I18nService.t('pub_empty_results_desc')}</p>
        </div>
      `;
      return;
    }
    ...
    const currency = (this.state.info?.currency || 'MXN').toUpperCase();
        ...
        <div class="pub-card" style="animation-delay:${Math.min(idx * 40, 400)}ms;">
          <a href="#/${encodedCompanyId}/producto/${p.id}" class="pub-card-img-wrapper" aria-label="Ver ${p.name}">
            ...
            <div class="pub-card-badges">
              ...
              ${isOutOfStock ? `<span class="pub-badge pub-badge-out">${I18nService.t('pub_badge_out_of_stock')}</span>` : ''}
            </div>
            ${isOutOfStock ? `<div class="pub-card-out-overlay"><span>${I18nService.t('pub_overlay_out_of_stock')}</span></div>` : ''}
            <button class="pub-fav-btn ${isFav ? 'active' : ''}" data-id="${p.id}"
              title="${isFav ? I18nService.t('pub_fav_remove') : I18nService.t('pub_fav_add')}"
              aria-label="${isFav ? I18nService.t('pub_fav_remove') : I18nService.t('pub_fav_add')}">
              ${isFav ? '♥' : '♡'}
            </button>
          </a>
          <div class="pub-card-body">
            ...
            <div class="pub-card-footer">
              ...
              <a href="#/${encodedCompanyId}/producto/${p.id}" class="pub-view-btn" style="text-decoration:none;"
                aria-label="${I18nService.t('pub_view_more')} de ${p.name}">
                ${I18nService.t('pub_view_more')}
              </a>
            </div>
          </div>
        </div>
