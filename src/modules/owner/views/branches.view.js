...
export class BranchesView extends Component {
  ...
  renderView(element) {
    ...
    if (q('#kpi-selected-context')) {
      ...
      q('#kpi-selected-context').textContent = selectedBranchId === 'all' ? 'Consolidado (Todas)' : (activeObj?.name || 'Sucursal');
    }
    if (q('#branch-count-label')) q('#branch-count-label').textContent = `Mostrando ${filtered.length} de ${branches.length} sucursal(es)`;
    ...
    if (filtered.length === 0) {
      container.innerHTML = `
        <div class="card p-10 text-center text-secondary">
          ...
          <h4 class="font-bold text-white text-lg">No se encontraron sucursales</h4>
          <p class="text-xs mt-1">Ajusta los filtros de búsqueda o registra una nueva sucursal con el botón superior.</p>
        </div>
      `;
      return;
    }
      ...
      <div class="card card-interactive p-5 flex flex-col justify-between ${isSelected ? 'border-indigo-500 shadow-indigo-500/20' : ''}" style="${isSelected ? 'background: rgba(99, 102, 241, 0.08); border-color: rgba(99, 102, 241, 0.4);' : ''}">
        <div>
          ...
          <h4 class="text-lg font-bold text-white mb-1 flex items-center gap-2">
            ${branch.name}
            ${branch.isDefault ? '<span class="text-xs text-amber-400 bg-amber-500/10 px-2 py-0.5 rounded border border-amber-500/20">Principal</span>' : ''}
          </h4>
          <p class="text-xs text-secondary mb-3">
            ${branch.city || 'General'}${branch.state ? `, ${branch.state}` : ''} &bull; ${branch.address || 'Sin dirección registrada'}
          </p>
          ...
          <div class="text-xs text-secondary space-y-1 py-3 border-t border-b border-gray-800/80 my-3">
            <div class="flex justify-between">
              <span>Encargado:</span>
              <strong class="text-slate-300">${branch.managerName || 'Sin asignar'}</strong>
            </div>
            <div class="flex justify-between">
              <span>Horario:</span>
        ...
        <div class="flex items-center gap-2 mt-2 pt-2">
          <button class="btn ${isSelected ? 'btn-primary' : 'btn-secondary'} btn-xs flex-1 btn-select-branch" data-id="${branch.id}" data-name="${branch.name}">
            ${isSelected ? '✓ Contexto Activo' : 'Seleccionar'}
          </button>
          <button class="btn btn-secondary btn-xs btn-edit-branch" data-id="${branch.id}" title="Editar sucursal">
          ...
          <button class="btn btn-secondary btn-xs btn-toggle-branch" data-id="${branch.id}" data-status="${branch.status}" title="${isActive ? 'Desactivar sucursal' : 'Activar sucursal'}">
            ${isActive ? 'Inactivar' : 'Activar'}
          </button>
          ${!branch.isDefault ? `
            <button class="btn btn-danger btn-xs btn-delete-branch" data-id="${branch.id}" data-name="${branch.name}" title="Eliminar sucursal">
      ...
      <div class="space-y-4 text-left">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div class="form-group">
            <label class="form-label" for="branch-form-name">Nombre de Sucursal *</label>
            <input type="text" id="branch-form-name" class="input input-md" placeholder="Ej. Pizza Cat León 1" value="${b.name || ''}" required />
          </div>
          <div class="form-group">
            <label class="form-label" for="branch-form-code">Código Interno *</label>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
          <div class="form-group">
            <label class="form-label" for="branch-form-country">País</label>
            <input type="text" id="branch-form-country" class="input input-md" value="${b.country || 'Nicaragua'}" />
          </div>
          <div class="form-group">
            <label class="form-label" for="branch-form-state">Departamento / Estado</label>
            <input type="text" id="branch-form-state" class="input input-md" placeholder="Ej. León" value="${b.state || ''}" />
          </div>
          <div class="form-group">
            <label class="form-label" for="branch-form-city">Municipio / Ciudad *</label>
            <input type="text" id="branch-form-city" class="input input-md" placeholder="Ej. León" value="${b.city || ''}" required />
          </div>
        </div>
        <div class="form-group">
          <label class="form-label" for="branch-form-address">Dirección Exacta</label>
          <input type="text" id="branch-form-address" class="input input-md" placeholder="Ej. De la Iglesia La Recolección 1c al sur" value="${b.address || ''}" />
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div class="form-group">
            <label class="form-label" for="branch-form-phone">Teléfono / WhatsApp</label>
            <input type="tel" id="branch-form-phone" class="input input-md" placeholder="+505 8888 8888" value="${b.phone || b.whatsapp || ''}" />
          </div>
          <div class="form-group">
            <label class="form-label" for="branch-form-email">Correo de Sucursal</label>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
          <div class="form-group">
            <label class="form-label" for="branch-form-manager">Encargado de Sucursal</label>
            <input type="text" id="branch-form-manager" class="input input-md" placeholder="Ej. Juan Pérez" value="${b.managerName || ''}" />
          </div>
          <div class="form-group">
            <label class="form-label" for="branch-form-opening">Horario Apertura</label>
            <input type="time" id="branch-form-opening" class="input input-md" value="${b.openingHours || '08:00'}" />
          </div>
          <div class="form-group">
            <label class="form-label" for="branch-form-closing">Horario Cierre</label>
            <input type="time" id="branch-form-closing" class="input input-md" value="${b.closingHours || '18:00'}" />
          </div>
        </div>
        <div class="form-group">
          <label class="form-label" for="branch-form-notes">Notas Internas</label>
          <textarea id="branch-form-notes" class="input input-md" rows="2" placeholder="Instrucciones específicas de operación...">${b.notes || ''}</textarea>
        </div>
      </div>
    `;

    const modal = new Modal({
      title: isEdit ? `✏️ Editar Sucursal: ${b.name}` : '🏢 Registrar Nueva Sucursal',
      bodyHTML,
      footerHTML: `
        <button class="btn btn-secondary btn-sm" id="btn-modal-branch-cancel">Cancelar</button>
        <button class="btn btn-primary btn-sm" id="btn-modal-branch-save">${isEdit ? 'Guardar Cambios' : 'Crear Sucursal'}</button>
      `
    });
  ...
  async confirmDeleteBranch(branchId, branchName) {
    ...
    if (employees.length > 0) {
      const optionsOtherBranches = otherBranches.map(b => 
        `<option value="${b.id}">${b.name} (${b.city || 'General'})</option>`
      ).join('');
      ...
      employeeActionHTML = `
        <div style="background: rgba(239, 68, 68, 0.08); border: 1px solid rgba(239, 68, 68, 0.25); border-radius: var(--radius-md); padding: 14px; margin-top: 12px;">
          <div style="font-weight: 700; color: #ef4444; font-size: 0.85rem; margin-bottom: 6px;">
            ⚠️ Esta sucursal tiene ${employees.length} empleado(s) registrado(s):
          </div>
          ...
          <div style="font-weight: 600; font-size: 0.82rem; color: #fff; margin-bottom: 8px;">
            ¿Qué deseas hacer con estos empleados?
          </div>
          ...
          ${otherBranches.length > 0 ? `
            <label style="display: flex; align-items: flex-start; gap: 8px; font-size: 0.8rem; color: #e2e8f0; cursor: pointer; margin-bottom: 8px;">
              ...
              <div>
                <strong>🔁 Trasladar empleados a otra sucursal:</strong>
          ...
          <label style="display: flex; align-items: flex-start; gap: 8px; font-size: 0.8rem; color: #e2e8f0; cursor: pointer;">
            ...
            <div>
              <strong style="color: #ef4444;">🗑️ Eliminar los empleados de Firebase permanentemente</strong>
              <div style="font-size: 0.73rem; color: var(--color-text-tertiary);">Se eliminarán las cuentas de acceso de estos colaboradores.</div>
            </div>
          </label>
        </div>
      `;
    ...
    const bodyHTML = `
      <div style="text-align: left; padding: 4px 0;">
        <p style="font-size: 0.9rem; color: var(--color-text-primary); margin-bottom: 8px;">
          ¿Estás seguro de que deseas eliminar la sucursal <strong style="color: #6366f1;">${branchName}</strong>?
        </p>
        <p style="font-size: 0.8rem; color: var(--color-text-secondary);">
          Esta acción eliminará el registro de la sucursal de Firebase en tiempo real.
        </p>
        ${employeeActionHTML}
      </div>
    `;
    ...
    const modal = new Modal({
      title: `🗑️ Eliminar Sucursal: ${branchName}`,
      bodyHTML,
      footerHTML: `
        <button class="btn btn-secondary btn-sm" id="btn-cancel-del-branch">Cancelar</button>
        <button class="btn btn-danger btn-sm" id="btn-confirm-del-branch">Eliminar Sucursal</button>
      `
    });
